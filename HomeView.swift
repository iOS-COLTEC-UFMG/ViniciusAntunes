...
struct HomeView: View {
    // ViewModel que monitora o áudio e fornece a classificação
    @StateObject private var audioVM = AudioLevelViewModel()

    var body: some View {
        ZStack {
            // Fundo adaptável ao estado
            ...
            VStack(spacing: 24) {
                ...
                // Indicador visual (ícone e valor em dB)
                VStack(spacing: 8) {
                    Image(systemName: iconForState(audioVM.levelState))
                        .resizable()
                ...
                // Cartão informativo (estado, descrição, dica)
                VStack(spacing: 16) {
        ...
        .onAppear {
            // Inicia o monitoramento de áudio ao entrar na tela
            audioVM.startMonitoring()
        }
        .onDisappear {
            // Para o monitoramento ao sair (boa prática)
